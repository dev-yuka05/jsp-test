	...
	<div class="wrap">
		...
		<section>
			...
			<% 
				...
				PreparedStatement pstmt=con.prepareStatement("select * from reservation_tbl WHERE lentno="+lentno);
			...
			<form method="GET" action="./edit-server.jsp">
				<table  border="1">
					<tr><th>대출번호</th><td><input name="lentno" value="<%=rs.getString(1) %>" required  readonly></td></tr>
					<tr><th>고객성명</th><td><input name="custname" value="<%=rs.getString(2) %>" required></td></tr>
					<tr><th>도서코드</th><td>
						<select name="bookno" required>
							<%
							  while(bookrs.next()){
								  if(rs.getString(3).equals(bookrs.getString(1))){
									  %>
									  	<option value="<%=bookrs.getString(1) %>" selected><%=bookrs.getString(3) %></option>
									  <%
								  }else{
								%>
					...
					<tr><th>대출일자</th><td><input name="outdate"  value="<%=rs.getString(4).substring(0,11) %>" required></td></tr>
					<tr><th>반납일자</th><td><input name="indate"  value="<%=rs.getString(5) != null?rs.getString(5).substring(0,11):"" %>"></td></tr>
					
					<tr><th>대출상태</th><td>
					
						<input name="status" type="radio" value="1" <%=rs.getInt(6)==1?"checked":""  %> required>대출 <input name="status" type="radio" value="2"  <%=rs.getInt(6)==1?"":"checked"  %> required>반납
